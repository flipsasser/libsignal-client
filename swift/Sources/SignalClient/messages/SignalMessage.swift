...
public class SignalMessage {
    ...
    deinit {
        assertNoError(signal_message_destroy(handle))
    }
    ...
    public init<MacBytes, CiphertextBytes>(version: UInt8,
                                           ...
                                           receiver receiverIdentityKey: PublicKey)
    where MacBytes: ContiguousBytes, CiphertextBytes: ContiguousBytes {
        handle = macKey.withUnsafeBytes { macBytes in
            ciphertext.withUnsafeBytes { ciphertextBytes in
                var result: OpaquePointer?
                assertNoError(signal_message_new(&result,
                                                 version,
