...
class PublicAPITests: TestCaseBase {
    ...
    func testAddress() {
        let addr = ProtocolAddress(name: "addr1", deviceId: 5)
    ...
    func testPkOperations() {
        ...
        XCTAssertEqual(pk.verifySignature(message: message, signature: signature), true)

        signature[5] ^= 1
        XCTAssertEqual(pk.verifySignature(message: message, signature: signature), false)
        signature[5] ^= 1
        XCTAssertEqual(pk.verifySignature(message: message, signature: signature), true)
        message[1] ^= 1
        XCTAssertEqual(pk.verifySignature(message: message, signature: signature), false)
    ...
    func testFingerprint() {
        ...
        let aliceFingerprint = generator.create(version: VERSION_1,
                                                localIdentifier: aliceStableId,
                                                localKey: aliceIdentityKey,
                                                remoteIdentifier: bobStableId,
                                                remoteKey: bobIdentityKey)

        let bobFingerprint = generator.create(version: VERSION_1,
        ...
        let aliceFingerprint2 = generator.create(version: VERSION_2,
                                                 localIdentifier: aliceStableId,
                                                 localKey: aliceIdentityKey,
                                                 remoteIdentifier: bobStableId,
                                                 remoteKey: bobIdentityKey)

        let bobFingerprint2 = generator.create(version: VERSION_2,
                                               localIdentifier: bobStableId,
                                               localKey: bobIdentityKey,
                                               remoteIdentifier: aliceStableId,
                                               remoteKey: aliceIdentityKey)
        ...
        let aliceFingerprintM = generator.create(version: VERSION_1,
                                                 localIdentifier: aliceStableId,
                                                 localKey: aliceIdentityKey,
                                                 remoteIdentifier: bobStableId,
                                                 remoteKey: mitmIdentityKey)

        let bobFingerprintM = generator.create(version: VERSION_1,
                                               localIdentifier: bobStableId,
                                               localKey: bobIdentityKey,
                                               remoteIdentifier: aliceStableId,
                                               remoteKey: aliceIdentityKey)
        ...
        let aliceFingerprintI = generator.create(version: VERSION_1,
                                                 localIdentifier: aliceStableId,
                                                 localKey: aliceIdentityKey,
                                                 remoteIdentifier: badBobStableId,
                                                 remoteKey: bobIdentityKey)

        let bobFingerprintI = generator.create(version: VERSION_1,
                                               localIdentifier: bobStableId,
                                               localKey: bobIdentityKey,
                                               remoteIdentifier: aliceStableId,
                                               remoteKey: aliceIdentityKey)
    ...
    func testGroupCipher() {
        ...
        let sender = ProtocolAddress(name: "+14159999111", deviceId: 4)
        let group_id = SenderKeyName(groupName: "summer camp", sender: sender)
